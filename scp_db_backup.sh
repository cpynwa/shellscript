...
## --- 로그기록시작
{
        ...
        /usr/bin/find /home/eknow/backup/IMS-WEB -mtime +7 -exec sh -c "ls -1 {};rm {}" \;
        /bin/echo

        ## 백업
        # DB dump
        /usr/bin/mysqldump eknow > /tmp/eknow.sql

        # p:퍼미션유지 P:절대경로유지
        /bin/tar czpPf "${BAK_FILE}" /etc/mysql/mysql.conf.d/mysqld.cnf /tmp/eknow.sql
        /usr/bin/scp -rp ${BAK_FILE} eknow@10.1.112.60:${BAK_FILE}
