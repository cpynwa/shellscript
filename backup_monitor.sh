...
WBK_CNT=$(/bin/ls -1 ${WEB} | /usr/bin/wc -1)
DBK_CNT1=$(/bin/ls -1 ${DB1} | /usr/bin/wc -1)
DBK_CNT2=$(/bin/ls -1 ${DB2} | /usr/bin/wc -1)
SBK_CNT=$(/bin/ls -1 ${STR} | /usr/bin/wc -1)
...
function chk_cnt() {
	...
	if [ "${CNT}" -eq 8 ]
	then
		/bin/echo "${NAME} 백업파일 문제없음."
		/bin/echo
	else
		/bin/echo "${NAME} 백업파일이 8개가 아닙니다! 확인하세요!"
		/bin/echo
	fi
}
...
## 레포팅 메세지 작성
{
	...
	/bin/echo
	/bin/echo "웹서버 백업파일"
	/bin/ls -1 ${WEB} | /bin/grep "${TODAY}"
	/bin/echo
	/bin/echo "DB서버 백업파일:"
	/bin/ls -1 ${DB1} | /bin/grep "${TODAY}"
	/bin/ls -1 ${DB2} | /bin/grep "${TODAY}"
	/bin/echo
	/bin/echo "Storage서버 백업파일:"
	/bin/ls -1 ${STR} | /bin/grep "${TODAY}"
	/bin/echo
